...
class FinalizeCaseDialog(QDialog, Ui_FinalizeCaseDialog, DatabaseHelper):
            ...
            try:
                ...
                for right_result in result:
                    ...
                    if right_result[0] == UserRight.cadastre_update:
                        self.land_office_cbox.addItem(land_office.user_name, land_office.user_name)
    ...
    def on_ok_button_clicked(self):
        ...
        self.maintenance_case.completed_by = user.user_name
    ...
    def __write_changes(self):
        ...
        try:
            ...
            #     filter(CaMaintenanceCase.id == self.maintenance_case.id).\
            #     filter(CaParcel.geometry.ST_Transform(int(srid)).ST_Contains(CaTmpParcel.geometry.ST_Transform(int(srid)).ST_Buffer(-0.005))). \
            #     filter(CaParcel.geometry.ST_Transform(int(srid)).ST_Area() != CaTmpParcel.geometry.ST_Transform(int(srid)).ST_Area()).all()
            #
            # for old_parcel in historical_parcels:
            #     old_parcel.valid_till = DatabaseUtils.convert_date(self.completion_date_edit.date())
            ...
            #     filter(CaTmpParcel.parcel_id.startswith(str(self.maintenance_case.id) + '-')).all()
            #
            ...
            #     parcel_inserted.valid_from = DatabaseUtils.convert_date(self.completion_date_edit.date())
            #     parcel_inserted.geometry = tmp_parcel.geometry
            #     parcel_inserted.landuse = tmp_parcel.landuse
            #     self.maintenance_case.parcels.append(parcel_inserted)
    ...
    def on_help_button_clicked(self):
        ...
        os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/cadastre_finalize_case.htm")
