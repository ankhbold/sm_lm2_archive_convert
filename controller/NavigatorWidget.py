...
class NavigatorWidget(QDockWidget, Ui_NavigatorWidget, DatabaseHelper):
    ...
    def __setup_combo_boxes(self):
        ...
        try:
            ...
            cl_app_type = self.session.query(ClApplicationType).all()
    ...
    def __create_person_view(self):
        ...
        for au_level2 in au_level2_list:
            ...
            select = "SELECT person.name, person.first_name, person.middle_name, person.person_id, person.state_registration_no, " \
                     "person.mobile_phone, person.phone, person.id, parcel.parcel_id, tmp_parcel.parcel_id tmp_parcel_id, application.app_no, decision.decision_no, " \
    ...
    def __create_application_view(self):
        ...
        for au_level2 in au_level2_list:
            ...
            select = "SELECT application.app_no, application.app_type, status.status, status.status_date, status.officer_in_charge, status.next_officer_in_charge, decision.decision_no, " \
                     "contract.contract_no, person.person_id, person.id as person_pk_id, person.name, person.first_name, person.middle_name, parcel.parcel_id, tmp_parcel.parcel_id tmp_parcel_id, record.record_no " \
    ...
    def __create_decision_view(self):
        ...
        for au_level2 in au_level2_list:
            ...
            select = "SELECT decision.decision_no, decision.decision_date, person.person_id, person.id as person_pk_id, person.name, person.middle_name, person.first_name, parcel.parcel_id, application.app_no, contract.contract_no, record.record_no " \
    ...
    def __create_contract_view(self):
        ...
        for au_level2 in au_level2_list:
            ...
            select = "SELECT contract.contract_no, contract.contract_date, person.person_id, person.id as person_pk_id, person.name, person.middle_name, person.first_name, parcel.parcel_id, application.app_no, decision.decision_no " \
    ...
    def __create_record_view(self):
            ...
            for au_level2 in au_level2_list:
                ...
                select = "SELECT record.record_no, record.record_date, person.person_id, person.id as person_pk_id, person.name, person.middle_name, person.first_name, parcel.parcel_id, application.app_no, decision.decision_no " \
    ...
    def __create_parcel_view(self):
            ...
            for au_level2 in au_level2_list:
                ...
                select = "SELECT parcel.parcel_id, parcel.old_parcel_id, parcel.geo_id, parcel.landuse, person.person_id, person.id as person_pk_id, person.name, person.middle_name, person.first_name,  application.app_no, decision.decision_no, contract.contract_no, record.record_no " \
        ...
        try:
            ...
            if self.office_in_charge_cbox.currentIndex() != -1:
                if not self.office_in_charge_cbox.itemData(self.office_in_charge_cbox.currentIndex()) == -1:
                    filter_is_set = True
                    officer = "%" + self.office_in_charge_cbox.itemData(self.office_in_charge_cbox.currentIndex()).user_name + "%"
            ...
            if self.next_officer_in_charge_cbox.currentIndex() != -1:
                if not self.next_officer_in_charge_cbox.itemData(self.next_officer_in_charge_cbox.currentIndex()) == -1:
                    filter_is_set = True
                    officer = "%" + self.next_officer_in_charge_cbox.itemData(self.next_officer_in_charge_cbox.currentIndex()).user_name + "%"
    ...
    def __search_parcels(self):
        ...
        if self.is_parcel_checkbox.isChecked():
            ...
            for parcel in parcels.distinct(ParcelSearch.parcel_id).all():
                ...
                item = QTableWidgetItem(parcel.parcel_id + self.tr(" ( Geo Id: ") + geo_id + self.tr(" )"))
    ...
    def on_record_delete_button_clicked(self):
        ...
        if len(items) > 1:
            ...
            message_box.setText(self.tr("Do you want to delete the record {0}").format(str(items[0].data(Qt.UserRole))))
    ...
    def on_case_create_button_clicked(self):
        ...
        ca_maintenance_case.created_by = DatabaseUtils.current_user().user_name
        session.add(ca_maintenance_case)
        print ca_maintenance_case.id
        DatabaseUtils.set_working_schema()
    ...
    def __report_setup(self):
        ...
        try:
            application_type = self.session.query(ClApplicationType).all()
    ...
    def __app_steps_progress(self):
        ...
        end_date = end_date + timedelta(days=1)
        try:
            app_type = self.session.query(ClApplicationType).all()
        ...
        try:
            ...
            for app in application:
                ...
                if app_person_count == 1:
                    ...
                    if app_person.person != None:
                        if app_person.person_ref.type == 10 or app_person.person_ref.type == 20 or app_person.person_ref.type == 50:
                            if app_person.person_ref.first_name != None:
                                first_name = app_person.person_ref.first_name
                            if app_person.person_ref.name != None:
                                name = app_person.person_ref.name
                ...
                if app_contract_count == 1 and contract.contract_end != None and contract.contract_end >= begin_date and contract.contract_end <= end_date:
                    if not self.report_person_type_cbox.itemData(self.report_person_type_cbox.currentIndex()) == -1:
                        if app_person.person_ref.type == self.report_person_type_cbox.itemData(self.report_person_type_cbox.currentIndex()):
                            worksheet.write(row, col, row_number,format)
                            worksheet.write(row, col+1,app.parcel_ref.parcel_id,format)
                            worksheet.write(row, col+2,app.parcel_ref.old_parcel_id,format)
                            worksheet.write(row, col+3,parcel_address,format)
                    ...
                    for person in app_person:
                        ...
                        if tax_zone_count == 1:
                            ...
                            tax_count = self.session.query(SetBaseTaxAndPrice).\
                                filter(SetBaseTaxAndPrice.tax_zone == tax_zone.fid).\
                        ...
                        if person.person_ref.address_au_level3 != None:
                            person_a3 = self.session.query(AuLevel3).filter(AuLevel3.code == person.person_ref.address_au_level3).one()
                            person_address_a3 = person_a3.name
                        if person.person_ref.address_au_level2 != None:
                            person_a2 = self.session.query(AuLevel2).filter(AuLevel2.code == person.person_ref.address_au_level2).one()
                            person_address_a2 = person_a2.name
                        if person.person_ref.address_au_level1 != None:
                            person_a1 = self.session.query(AuLevel1).filter(AuLevel1.code == person.person_ref.address_au_level1).one()
                            person_address_a1 = person_a1.name
                        if person.person_ref.address_street_name != None:
                            person_address_street_name = person.person_ref.address_street_name
                        if person.person_ref.address_khaskhaa != None:
                            person_address_khaskhaa = person.person_ref.address_khaskhaa

                        if person.person_ref.type == 10 or person.person_ref.type == 20 \
                        ...
                        if person.person_ref.type == 30 or person.person_ref.type == 40 \
                            or person.person_ref.type == 60:
                            first_name = person.person_ref.name

                        if person.role == Constants.LEGAL_REP_ROLE_CODE:
                            legal_surname = person.person_ref.name
                            legal_name = person.person_ref.first_name
                            legal_person_id = person.person_ref.person_id
                        ...
                        elif person.role == Constants.GIVING_UP_OWNER_CODE:
                            owner_change = u'Ха'
                            ...
                            worksheet.write(row, col+13, person.person_ref.person_id, format)
        ...
        try:
            ...
            for value in values:
                ...
                for person in app_person:
                    ...
                    if tax_zone_count == 1:
                        tax_zone = self.session.query(SetTaxAndPriceZone).filter(SetTaxAndPriceZone.geometry.ST_Contains(value.CtApplication.parcel_ref.geometry)).one()
                        tax_count = self.session.query(SetBaseTaxAndPrice).filter(SetBaseTaxAndPrice.tax_zone == tax_zone.fid).count()
                        if tax_count == 1:
                            tax = self.session.query(SetBaseTaxAndPrice).filter(SetBaseTaxAndPrice.tax_zone == tax_zone.fid).one()
                            price = str(float(tax.base_value_per_m2) * value.CtApplication.parcel_ref.area_m2)
                    if person.person_ref.address_au_level3 != None:
                        person_a3 = self.session.query(AuLevel3).filter(AuLevel3.code == person.person_ref.address_au_level3).one()
                        person_address_a3 = person_a3.name
                    ...
                    for person in app_person:
                        ...
                        if tax_zone_count == 1:
                            ...
                            if tax_count == 1:
                                ...
                                tax_c = self.session.query(CtTaxAndPrice).filter(CtTaxAndPrice.person == person.person_ref.person_id)\
                                                                        .filter(CtTaxAndPrice.record == value.CtOwnershipRecord.record_no).count()
                                if tax_c == 1:
                                    tax = self.session.query(CtTaxAndPrice).filter(CtTaxAndPrice.person == person.person_ref.person_id)\
    ...
    def __land_fee_quarterly(self):
        ...
        worksheet.merge_range('D2:L2', a1 + u' аймгийн ' + a2 + u' сумын хагас, бүтэн жилийн газрын төлбөрийн тайлан',format_header)
        worksheet.merge_range('A4:A6', u'Д/д',format)
        worksheet.merge_range('B4:J4', u'Нэгж талбарын мэдээлэл', format)
        worksheet.merge_range('C5:D5', u'Газрын хэмжээ', format)
        worksheet.merge_range('G5:J5', u'Газрын байршлын нэр, хаяг', format)
                    ...
                    for person in app_person:
                        ...
                        if tax_zone_count == 1:
                            ...
                            tax_count = self.session.query(SetBaseFee).\
                                filter(SetBaseFee.fee_zone == tax_zone.fid).\
                                filter(SetBaseFee.landuse == value.CtApplication.approved_landuse).count()
                            if tax_count == 1:

                                payment_year = self.year_sbox.value()
                                fee_c = self.session.query(CtFee).filter(CtFee.person == person.person_ref.person_id)\
                                ...
                                if fee_c == 1 and (value.CtContract.contract_end.year-value.CtContract.contract_begin.year) > 1:
                                    fee = self.session.query(CtFee).filter(CtFee.person == person.person_ref.person_id)\
    ...
    def __auction_info(self):
        ...
        worksheet.merge_range('B4:B6', u'Аймаг, сумын нэр', format)
    ...
    def __condominium_area(self):
        ...
        worksheet.merge_range('B4:B5', u'Аймаг, сумын нэр', format)
    ...
    def on_parcel_results_twidget_itemClicked(self, item):
        ...
        if self.is_parcel_checkbox.isChecked():
            ...
            self.parcel_num_edit.setText(parcel_result.parcel_id)
            self.geo_id_edit.setText(parcel_result.geo_id)
            # app_count = self.session.query(CtApplication).filter(CtApplication.tmp_parcel == id).count()
            # if app_count > 0:
            #     apps = self.session.query(CtApplication).filter(CtApplication.tmp_parcel == id).all()
            #     for app in apps:
            #         app_persons = self.session.query(CtApplicationPersonRole).\
            #             filter(CtApplicationPersonRole.application == app.app_no).all()
            #         for app_person in app_persons:
            #             if app_person.main_applicant == True:
            #                 person = self.session.query(BsPerson.)
            #     app_persons = self.session
            self.parcel_right_holder_name_edit.setText(parcel_result.first_name)
            self.parcel_app_num_edit.setText(parcel_result.app_no)
            self.parcel_decision_num_edit.setText(parcel_result.decision_no)
            if parcel_result.contract_no != None:
    ...
    def on_contract_results_twidget_itemClicked(self, item):
        ...
        id = item.data(Qt.UserRole)
        try:
            contract_result = self.session.query(ContractSearch).filter(ContractSearch.contract_no == id).one()
            self.contract_num_edit.setText(str(contract_result.contract_no))
            self.contract_date.setDate(contract_result.contract_date)
    ...
    def on_record_results_twidget_itemClicked(self, item):
        ...
        try:
            ...
            self.record_record_num_edit.setText(str(record_result.record_no))
