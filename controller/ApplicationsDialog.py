...
class ApplicationsDialog(QDialog, Ui_ApplicationsDialog, DatabaseHelper):
    ...
    def __setup_mapping(self):
        ...
        try:
            ...
            contract_count = self.session.query(CtContractApplicationRole)\
    ...
    def __setup_combo_boxes(self):
        ...
        for setRole in set_roles:
            self.next_officer_in_charge_cbox.addItem(setRole.surname + ", " + setRole.first_name, setRole.user_name)
    ...
    def on_accept_parcel_number_button_clicked(self):
        ...
        for row in range(num_rows):
            ...
            # app_person_count = self.session.query(CtApplication)\
            ...
            #     .filter(CtApplicationPersonRole.person == bs_person.id).count()
            # if app_person_count > 0:
            #     PluginUtils.show_message(self, self.tr("Parcel Duplicated"), self.tr("This parcel is already registered on this person"))
            #     return
        ...
        if app_case_count > 0:
            ...
            return
        if self.application_type_cbox.itemData(self.application_type_cbox.currentIndex()) == 2:
            ...
            if applicants_count < 2:
                PluginUtils.show_message(self, self.tr("Applicant error"), self.tr("This application have two or more applicant!!!"))
                return
        ...
        if used_parcel_count > 0 and current_app_type != 8 and current_app_type != 11:
            # used_parcel = self.session.query(CtApplication,BsPerson).join(CtDecisionApplication, CtApplication.app_no == CtDecisionApplication.application)\
            #                 .join(CtApplicationPersonRole, CtApplication.app_no == CtApplicationPersonRole.application)\
        ...
        if tax_zone_count != 0:
            ...
            if price_area_count != 0:
                ...
                excess_area = round(parcel.area_m2)-round(price_area.subsidized_area)
                if excess_area > 0:
        ...
        for item in self.navigator.person_results_twidget.selectedItems():
            ...
            try:
                if application_type_code == 1:
                    first_owner_count = self.session.query(CtApplicationPersonRole)\
                        ...
                        .filter(CtApplication.app_type == 1).count()
    ...
    def on_update_button_clicked(self):
        ...
        for appStatus in self.application.statuses:
            ...
            if appStatus.status_ref.code == status_item.data(Qt.UserRole):
                ...
                next_officer_item.setData(Qt.UserRole, appStatus.next_officer_in_charge_ref.user_name)
    ...
    def on_add_button_clicked(self):
        ...
        try:
            officer = self.session.query(SetRole).filter_by(user_name=QSettings().value(SettingsConstants.USER)).one()
            next_officer = self.session.query(SetRole).filter_by(user_name=next_officer_username).one()
    ...
    def reject(self):
        ...
        if QDateTime.currentDateTime().toString("yyMMdd") in self.application.app_no:
    ...
    def __add_app_remarks(self,map_composition):
        ...
        #             person = self.session.query(BsPerson).filter(BsPerson.id == p.person).one()
        # except SQLAlchemyError, e:
        #     raise LM2Exception(self.tr("Database Query Error"), self.tr("aCould not execute: {0}").format(e.message))
        ...
        #     for p in app_status:
        #         if p.status == 1:
        #             officer = self.session.query(SetRole).filter(SetRole.user_name == p.officer_in_charge).one()
        # except SQLAlchemyError, e:
        #     raise LM2Exception(self.tr("Database Query Error"), self.tr("aCould not execute: {0}").format(e.message))
        #
        # person_name = ""
        ...
        # else:
        #     person_name = person.name +u", "+ person.first_name
        # text = person_name + u' -ны бүртгэлийн № ' + app_no + u' -д бүртгэгдсэн ___ хуудас өргөдлийг хүлээн авч, хянан '\
        #         + app.remarks + u' үндэслэлээр таны гаргасан өргөдлийг буцаав. ' + aimag.name + u' аймгийн ' + soum.name \
        #         + u' сумын газрын асуудал эрхэлсэн албан тушаалтан ' + officer.surname + u' ,' + officer.first_name
        #
        ...
        for item in self.navigator.person_results_twidget.selectedItems():
            ...
            try:
                ...
                if person.type == Constants.UNCAPABLE_PERSON_TYPE \
                        or person.type == Constants.FOREIGN_ENTITY_PERSON_TYPE \
                        ...
                        PluginUtils.show_error(self, self.tr("Error"),
                                               self.tr("Its not allowed to add incapable persons, "
                                                                               "#state organisations or entities of "
    ...
    def __copy_contract_from_navigator(self):
        ...
        contract_app = self.session.query(CtContractApplicationRole).\
            ...
            filter(CtApplication.app_type != 8).one()
        ...
        app_role_create = contract.application_roles\
                                .filter(CtContractApplicationRole.role==Constants.APPLICATION_ROLE_CREATES).one()
        # if app_role_create.application_ref.parcel != self.parcel_edit.text():
        #     PluginUtils.show_error(self, self.tr("Error"),
        #                            self.tr("The contract {0} has another parcel assignment.").format(contract_no))
        #     return
        ...
        contract_fee = self.session.query(CtFee).filter(CtFee.contract == contract_no).all()
        for fee in contract_fee:
            applicants = self.session.query(CtApplicationPersonRole).\
                join(CtContractApplicationRole, CtApplicationPersonRole.application == CtContractApplicationRole.application).\
                filter(CtContractApplicationRole.contract == contract_no).\
                filter(CtApplicationPersonRole.person == fee.person).all()
    ...
    def __copy_applicant_from_navigator(self):
        ...
        #    for owner in selected_remain_persons:
        #        self.__add_old_right_holder_item(owner)
    ...
    def on_help_button_clicked(self):
        ...
        if app_type == 02:
            if self.application_tab_widget.currentIndex() == 0:
                ...
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_change_of_co-ownership.htm")
            elif self.application_tab_widget.currentIndex() == 5:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_record_cancelled.htm")
        ...
        if app_type == 03:
            if self.application_tab_widget.currentIndex() == 0:
                ...
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_legal_representative.htm")
            elif self.application_tab_widget.currentIndex() == 5:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_record_created.htm")
            elif self.application_tab_widget.currentIndex() == 6:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_print.htm")
        ...
        if app_type == 07:
            if self.application_tab_widget.currentIndex() == 0:
                ...
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_transfer_of_right.htm")
            elif self.application_tab_widget.currentIndex() == 5:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_contract_created.htm")
            elif self.application_tab_widget.currentIndex() == 6:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_contract_cancelled.htm")
            elif self.application_tab_widget.currentIndex() == 7:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_print.htm")
        if app_type == 8:
            if self.application_tab_widget.currentIndex() == 0:
                ...
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_relating_contract.htm")
            elif self.application_tab_widget.currentIndex() == 5:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_mortgagee.htm")
            elif self.application_tab_widget.currentIndex() == 6:
                os.system("hh.exe "+ str(os.path.dirname(os.path.realpath(__file__))[:-10]) +"help\output\help_lm2.chm::/html/application_print.htm")
        if app_type == 9:
            if self.application_tab_widget.currentIndex() == 0:
