...
class ImportDecisionDialog(QDialog, Ui_ImportDecisionDialog, DatabaseHelper):
    ...
    def __read_xls_file(self):
        ...
        user = DatabaseUtils.current_user()
        self.import_by_edit.setText(user.user_name)
        ...
        self.decision.imported_by = DatabaseUtils.current_user().user_name
        ...
        for row in range(1, s.nrows):
            ...
            if self.is_tmp_parcel:
                ...
                if not application_result_exists:
                    ...
                    user = DatabaseUtils.current_user()
                    app_status.next_officer_in_charge = user.user_name
                    app_status.officer_in_charge = user.user_name
        ...
        # try:
            ...
            if buildings_to_be_inserted_count != 0:
                buildings_to_be_inserted = self.session.query(CaTmpBuilding) \
                    ...
                    .filter(CaTmpParcel.geometry.ST_Intersects(CaTmpBuilding.geometry))\
                ...
                for tmp_building in buildings_to_be_inserted:
                    ...
                    self.session.query(CaTmpBuilding). \
                        filter(CaTmpBuilding.maintenance_case == maintenance_case_id). \
                        filter(~CaTmpBuilding.geometry.ST_Equals(tmp_building.geometry)).delete()
    ...
    def on_add_document_button_clicked(self):
        ...
        if file_dialog.exec_():
            ...
            try:
                dec_doc = CtDecisionDocument()
                dec_doc.decision = self.decision.decision_no
                dec_doc.decision_ref = self.decision
                ...
                dec_doc.document_ref = doc
                dec_doc.document = doc.id
                self.decision.documents.append(dec_doc)

                item = QListWidgetItem(file_name, self.document_twidget)
                item.setData(Qt.UserRole, dec_doc)
    ...
    def on_delete_document_button_clicked(self):
        ...
        try:
            dec_document_item = item.data(Qt.UserRole)
            dec_document = self.decision.documents\
                .filter(CtDecisionDocument.document_ref == dec_document_item.document_ref)\
                .filter(CtDecisionDocument.decision == dec_document_item.decision).one()

            self.session.delete(dec_document)
    ...
    def on_view_document_button_clicked(self):
        ...
        dec_doc = byte_array_item.data(Qt.UserRole)
        ...
        if byte_array is None:
            ...
            current_bytes = QByteArray(byte_array)
    ...
    def on_decision_save_button_clicked(self):
        ...
        user_name = QSettings().value(SettingsConstants.USER)
        working_soum = self.session.query(SetRole).filter(SetRole.user_name == user_name).one()
        ...
        if self.seller_buyer_checkbox.isChecked():
            ...
            # app_no = self.application_cbox.currentText()
            # application = self.session.query(CtApplication).filter(CtApplication.app_no == )
            # self.session.add(app_status)
            #
            # self.__write_changes(m, )
        # except SQLAlchemyError, e:
        #     self.rollback()
        #     PluginUtils.show_error(self, self.tr("Query Error"), self.tr("Error in line {0}: {1}").format(currentframe().f_lineno, e.message))
        #     return
    ...
    def on_person_id_edit_textChanged(self, text):
        ...
        application = self.session.query(CtApplication)\
            ...
            .filter(CtApplication.app_type == 15)\
    ...
    def __populate_combobox(self):
        ...
        for officer in officers:
                self.officer_by_find_cbox.addItem(officer.user_name, officer.user_name)
