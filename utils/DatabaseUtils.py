...
class DatabaseUtils():
    ...
    def current_user():
        ...
        try:
            set_role = session.query(SetRole).filter(SetRole.user_name == user).one()
            return set_role
    ...
    def role_settings(name):
        ...
        try:
            user_setting_result = session.query(SetRole).filter_by(user_name=name).one()
            session.commit()
        except exc.SQLAlchemyError, e:
            session.rollback()
            raise LM2Exception(QApplication.translate("LM2", "Database Query Error"),
    ...
    def set_working_schema(first_code=None):
        ...
        try:
            ...
            if l1_working_code[:2] == "01":
                search_path_array = DatabaseUtils.l1_restriction_array()
            else:
