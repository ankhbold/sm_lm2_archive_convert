...
class SessionHandler(QObject):
    ...
    def create_session(self, user, password, host, port, database):
        ...
        try:
            ...
            self.session.execute("SET search_path to base, codelists, admin_units, settings, public")

            set_role_count = self.session.query(SetRole).filter(SetRole.user_name == user).count()
            if set_role_count == 0:
            ...
            setRole = self.session.query(SetRole).filter(SetRole.user_name == user).one()
            ...
            self.session.execute("SET search_path to base, codelists, admin_units, settings, public" + ", " + schema_string)
            self.session.commit()
